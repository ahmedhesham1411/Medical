...
public class HospitalsViewModel extends ViewModel {
    ...
    HospitalsActivity activity;
    AlertDialog alertDialog;
    private CompositeSubscription mSubscriptions = new CompositeSubscription();
    LoginResponse loginResponse2;
    public SortBottomSheet sortBottomSheet;

    public HospitalsViewModel(HospitalsActivity activity) {
    ...
    public void GetHospitals() {
        ...
        mSubscriptions.add(NetworkUtil.getRetrofitByToken(Preferences.GetToken(activity))
                .GetHospitals(Locale.getDefault().getLanguage(),activity.pageIndex,Integer.parseInt(Preferences.GetCityId(activity)),true)
    ...
    public void GetHospitals2() {
        //showLoadingDialog(activity);
        mSubscriptions.add(NetworkUtil.getRetrofitByToken(Preferences.GetToken(activity))
                .GetHospitals(Locale.getDefault().getLanguage(),activity.pageIndex,Integer.parseInt(Preferences.GetCityId(activity)),false)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
