...
import com.gharbia.medical.Activities.ItemResultHospitalActivity;
...
public class ItemResultHospitalViewModel extends ViewModel {

    @SuppressLint("StaticFieldLeak")
    ItemResultHospitalActivity activity;
    SliderItemAdapter adapter;
    public ObservableField<Boolean> isReviews = new ObservableField<>(false);
    AlertDialog alertDialog;
    private CompositeSubscription mSubscriptions = new CompositeSubscription();
    public String faceBook,youTube,whatsApp,instagram,webSite,whatsappNumber,phone,phone2,name,img;


    public ItemResultHospitalViewModel(ItemResultHospitalActivity activity) {
        this.activity = activity;

        name = activity.getIntent().getStringExtra("name");
        activity.binding.nameTop.setText(name);
        GetDetails();
    ...
    public void GetDetails() {
        ...
        mSubscriptions.add(NetworkUtil.getRetrofitNoHeaderMultiPart()
                .GetHospitalDetails(activity.clinicId,Locale.getDefault().getLanguage())
