...
public class MedicalCentersViewModel extends ViewModel {

    @SuppressLint("StaticFieldLeak")
    MedicalCentersActivity activity;
    AlertDialog alertDialog;
    private CompositeSubscription mSubscriptions = new CompositeSubscription();
    LoginResponse loginResponse2;
    public SortBottomSheet sortBottomSheet;

    public MedicalCentersViewModel(MedicalCentersActivity activity) {
        this.activity = activity;

        GetMedicalCenters();
    }

    public void GetMedicalCenters() {
        showLoadingDialog(activity);
        mSubscriptions.add(NetworkUtil.getRetrofitByToken(Preferences.GetToken(activity))
                .GetMedicalCenters(Locale.getDefault().getLanguage(),activity.pageIndex,Integer.parseInt(Preferences.GetCityId(activity)),true)
    ...
    public void GetHospitals2() {
        //showLoadingDialog(activity);
        mSubscriptions.add(NetworkUtil.getRetrofitByToken(Preferences.GetToken(activity))
                .GetMedicalCenters(Locale.getDefault().getLanguage(),activity.pageIndex,Integer.parseInt(Preferences.GetCityId(activity)),false)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
    ...
    private void handleResponse2(HospitalsResponse hospitalsResponse) {
        alertDialog.dismiss();
        activity.updateRecycler(hospitalsResponse.getData());
    }
